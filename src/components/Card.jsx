...
const Card = ({ title, image, price, id, product, toggle, onClick, setProduct }) => {
  const { user } = useAuth()
  const { addToCart } = useContext(CartContext)
  const result = product.wishlists.filter( d => d.userId ===  user?user.uid:0)
  ...
  const handleaddwishlist = (e)=>{
    e.preventDefault();
    if(user){
      dispatch(create_new_wishlist({user_id: user.uid, product_id: id}))
    }else{
      alert("error")
    }
  ...
  return (
    <div className="group group-hover:bg-opacity-60 transition duration-500 relative bg-gray-50 sm:p-[5rem] py-36 px-10 flex justify-center items-center">
      ...
      <div className="flex flex-col bottom-8 left-8 space-y-4 absolute opacity-0 group-hover:opacity-100 transition duration-500">
        ...
        <button onClick={(e) => {toggle(); onClick(id); setProduct(product);}}>
          <svg width={24} height={24} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
